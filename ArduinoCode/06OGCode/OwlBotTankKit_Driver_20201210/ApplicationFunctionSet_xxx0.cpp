...
{
  // Keep track of whether we were moving last time
  ...
  uint16_t d = UltrasoundData_mm;

  // Treat these as invalid / error readings (common values seen: 65533, 65531, etc.)
  // (Also optional: ignore extremely large values that are clearly bogus)
  bool valid = (d < 5000) && (d != 65533) && (d != 65531) && (d != 65535);

  // Print (label invalid readings clearly)
  ...
  if (millis() - lastPrintMs >= 100)
  {
    ...
    Serial.print(d);
    if (!valid) Serial.print("  (invalid - ignored)");
    Serial.println();
  }

  // If invalid: do NOT change the motion state
  if (!valid)
  {
    // Keep current behavior exactly as-is:
    // - if it was moving, keep moving
    // - if it was stopped, keep stopped
    if (wasMoving)
