...
#define ULTRASONIC_ADDR 0x21

void DeviceDriverSet_ULTRASONIC::DeviceDriverSet_ULTRASONIC_Get(uint16_t *ULTRASONIC_Get /*out*/)
{
  // Many I2C sensors require setting a register pointer before reading.
  // We'll try register 0x00 (common for "data/high byte").
  Wire.beginTransmission((uint8_t)ULTRASONIC_ADDR);
  Wire.write((uint8_t)0x00);
  if (Wire.endTransmission(false) != 0)   // false = repeated-start (don’t release bus)
  {
    *ULTRASONIC_Get = 0xFFFF;
    return;
  }

  Wire.requestFrom((uint8_t)ULTRASONIC_ADDR, (uint8_t)2);
  if (Wire.available() >= 2)
  {
    uint8_t hi = Wire.read();
    uint8_t lo = Wire.read();
    *ULTRASONIC_Get = ((uint16_t)hi << 8) | lo;
  }
  else
  {
...
{
  unsigned dat[2] = {0};
  Wire.requestFrom(0x21, 1); //从器件读取一位数
  if (Wire.available() > 0)
  {
    dat[0] = Wire.read();
  }
